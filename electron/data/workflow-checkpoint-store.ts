...
type WorkflowNodeStatus = 'pending' | 'ready' | 'running' | 'completed' | 'failed' | 'blocked' | 'interrupted' | 'stopped'
type WorkflowExecutionStatus = 'queued' | 'running' | 'waiting-user' | 'interrupted' | 'completed' | 'stopped' | 'failed'
interface WorkflowRunnerState {
  status: WorkflowExecutionStatus
  nodes: Array<{ nodeId: string; status: WorkflowNodeStatus; attempt: number; error: string | null; output: string | null }>
