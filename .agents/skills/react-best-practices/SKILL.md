...
description: >
  ...
  Use when writing, reviewing, debugging, or refactoring React code for performance,
  including async waterfalls, bundle size, rendering, state updates, and client data access.
---
...
Read only the relevant files under `references/`. Use `rg` against that directory to find
rules by topic or prefix without loading the entire rule set.
